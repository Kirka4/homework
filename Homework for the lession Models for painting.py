...
# Код вы проверить не сможете так как у меня кончились бесплатные промты, так что остается либо поверить в то что оно работает либо зарегаться на https://dreamstudio.ai
# Кстати, изначально когда я еще не дошел до этого варианта, а код предложенный сайтом не работал(он должен был в репозитории где и код создавать файл и в него пихать картинку)
# Но почему то он файл не создавал, а если я его создавал то просто ничего не делал, поэтому
# я придумал гениальную вещь - брать картику и с помощью всяких ухищрений превращать ее в asci пиксель-арт, это было забавно но все же не то что бы хотелось
# вот такой код был:
...
# Потом мне пришла в голову идея просто брать картинку и по сути делать почти тоже самое только просто сохранять ее в репозитории с кодом
